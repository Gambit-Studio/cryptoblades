...
contract CryptoBlades is Initializable, AccessControlUpgradeable {
    ...
    function mintCharacter() public onlyNonContract oncePerBlock(msg.sender) {
        ...
        (, , uint256 fromUserWallet) = getSkillToSubtract(
            0,
            tokenRewards[msg.sender],
            skillAmount
        );
        require(
            skillToken.balanceOf(msg.sender) >= fromUserWallet &&
        ...
        isSupportedElement(chosenElement)
    {
        ...
        _payContractConvertedSupportingStaked(
            msg.sender,
            usdToSkill(mintWeaponFee * num * chosenElementFee)
        );
        _mintWeaponNLogic(num, chosenElement);
    }

    function mintWeapon(uint8 chosenElement)
        external
        onlyNonContract
        oncePerBlock(msg.sender)
        isSupportedElement(chosenElement)
    {
        ...
        _payContractConvertedSupportingStaked(
            msg.sender,
            usdToSkill(mintWeaponFee * chosenElementFee)
        );
    ...
    {
        uint8 chosenElementFee = chosenElement == 100 ? 1 : 2;
        int128 discountedMintWeaponFee = mintWeaponFee
    ...
    function mintWeaponUsingStakedSkill(uint8 chosenElement)
        external
        onlyNonContract
        oncePerBlock(msg.sender)
        isSupportedElement(chosenElement)
    ...
    function burnWeapon(uint256 burnID) external isWeaponOwner(burnID) {
        _payContractConvertedSupportingStaked(
            msg.sender,
            usdToSkill(burnWeaponFee)
        );

        _burnWeaponLogic(burnID);
    }

    function burnWeapons(uint256[] calldata burnIDs)
        external
    ...
    function reforgeWeaponWithDust(
        ...
        uint8 amount4B,
        uint8 amount5B
    ) external isWeaponOwner(reforgeID) {
        _payContractConvertedSupportingStaked(
            msg.sender,
            usdToSkill(reforgeWeaponWithDustFee)
        );

        _reforgeWeaponWithDustLogic(reforgeID, amountLB, amount4B, amount5B);
    }

    function burnWeaponUsingStakedSkill(uint256 burnID)
        external
        isWeaponOwner(burnID)
    {
        int128 discountedBurnWeaponFee = burnWeaponFee.mul(
            PAYMENT_USING_STAKED_SKILL_COST_AFTER_DISCOUNT
        );
    ...
    function burnWeaponsUsingStakedSkill(uint256[] calldata burnIDs)
        external
        isWeaponsOwner(burnIDs)
    {
        int128 discountedBurnWeaponFee = burnWeaponFee
    ...
    function reforgeWeaponUsingStakedSkill(uint256 reforgeID, uint256 burnID)
        external
        isWeaponOwner(reforgeID)
        isWeaponOwner(burnID)
    {
        int128 discountedReforgeWeaponFee = reforgeWeaponFee.mul(
            PAYMENT_USING_STAKED_SKILL_COST_AFTER_DISCOUNT
        );
        _payContractStakedOnly(
            msg.sender,
            usdToSkill(discountedReforgeWeaponFee)
        );

        _reforgeWeaponLogic(reforgeID, burnID);
    }

    function reforgeWeaponWithDustUsingStakedSkill(
        uint256 reforgeID,
        uint8 amountLB,
        uint8 amount4B,
        uint8 amount5B
    ) external isWeaponOwner(reforgeID) {
        int128 discountedReforgeWeaponWithDustFee = reforgeWeaponWithDustFee
            .mul(PAYMENT_USING_STAKED_SKILL_COST_AFTER_DISCOUNT);
        _payContractStakedOnly(
            msg.sender,
            usdToSkill(discountedReforgeWeaponWithDustFee)
        );
    ...
    function _reforgeWeaponWithDustLogic(
        uint256 reforgeID,
        uint8 amountLB,
        uint8 amount4B,
        uint8 amount5B
    ...
    function _isSupportedElement(uint8 element) internal pure {
        require(
            element == 100 || (element >= 0 && element <= 3),
    ...
    function _payContractTokenOnly(
        ...
        (
            ...
            uint256 fromTokenRewards,
            uint256 fromUserWallet
        ) = getSkillToSubtract(0, tokenRewards[playerAddress], convertedAmount);
        ...
        )
    {
        ...
        require(
            skillToken.balanceOf(playerAddress) >= fromUserWallet,
            string(
                abi.encodePacked(
                    "Not enough SKILL! Need ",
                    RandomUtil.uint2str(convertedAmount)
                )
            )
        );
    ...
    function _payContractConvertedSupportingStaked(
        address playerAddress,
        uint256 convertedAmount
    )
        internal
        ...
        )
    {
        ...
        (uint256 fromUserWallet, uint256 fromStaked) = getSkillToSubtractSingle(
            _remainder,
    ...
    function _payContractStakedOnly(
        address playerAddress,
        uint256 convertedAmount
    ) internal {
    ...
    function _deductPlayerSkillStandard(
        ...
        if (fromInGameOnlyFunds > 0) {
            totalInGameOnlyFunds = totalInGameOnlyFunds.sub(
                fromInGameOnlyFunds
            );
    ...
    function setReforgeWeaponValue(uint256 cents) public restricted {
        ...
        require(
            ...
            "Reforge fee must include burn fee"
        );
    ...
    function setRewardsClaimTaxMax(int128 _rewardsClaimTaxMax)
        public
        restricted
    {
    ...
    function setRewardsClaimTaxMaxAsRational(
        uint256 _numerator,
    ...
    function setRewardsClaimTaxMaxAsPercent(uint256 _percent)
        public
        restricted
    {
    ...
    function setRewardsClaimTaxDuration(uint256 _rewardsClaimTaxDuration)
        public
        restricted
    {
        ...
        returns (int128)
    {
        ...
        assert(
            0 <= durationUntilNoTax &&
                durationUntilNoTax <= rewardsClaimTaxDuration
        );

        int128 frac = ABDKMath64x64.divu(
            durationUntilNoTax,
            rewardsClaimTaxDuration
        );
